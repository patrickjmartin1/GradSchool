...
while k<=100 %This loop controls each discrete step
    ...
    if k == 1
        S(1).gamma = exp(-abs(y(1)-f(1)*1)^2);
        S(2).gamma = S(1).gamma;
        S(3).gamma = exp(-abs(y(1)-f(1)*(-1))^2);
        S(4).gamma = S(3).gamma;
        k =k+1;
        continue
    ...
    elseif k ==2 
        for j = 1:4
            S(j).gamma = S(j).gamma + exp(-abs(y(2)-f(1)*S(j).A(2)-f(2)*S(j).A(1))^2);
        ...
        for j = 1:4
            S(j).p = [S(j).V 1];
            S(j).n = [S(j).V -1];
            S(j).gamman = S(j).gamma + exp(-abs(y(k)-f(1)*S(j).n(end)-f(2)*S(j).n(end-1))^2);
            S(j).gammap = S(j).gamma + exp(-abs(y(k)-f(1)*S(j).p(end)-f(2)*S(j).p(end-1))^2);
